    ...
    .controller('MainController', ['$scope', '$http', ($scope, $http) => {
        ...
        $scope.submit = function(){
            ...
            $http.post('/postPhoto', formData, {
                ...
                if(res.data.warning){
                    Alert.warning('Warning', res.data.warning, {displayDuration: 5 * 1000})
                }else if (res.data.error) {
                    Alert.error('Error', res.data.error, {displayDuration: 5 * 1000})
                }else if (res.data.success) {
                    Alert.success('Success', res.data.success, {displayDuration: 5 * 1000})
                }
        ...
        $scope.thumb_up = function(data){
            $http.post('/thumbup', {data:data})
        }
        $scope.thumb_down = function(data){
            $http.post('/thumbdown', {data:data})
        }
